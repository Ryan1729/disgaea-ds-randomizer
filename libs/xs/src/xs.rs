...
pub fn shuffle<A>(xs: &mut Xs, slice: &mut [A]) {
    // The `as usize` casts below assume this is the case
    compile_time_assert!{
        usize::BITS >= u32::BITS
    }
    ...
    shuffle_with_swap(
        xs,
        slice.len() as u32,
        |i1, i2| slice.swap(i1 as usize, i2 as usize),
    );
}

pub fn shuffle_with_swap<Swap>(
    xs: &mut Xs,
    len: u32,
    mut swap: Swap,
)
where Swap: FnMut(u32, u32)
{
    for i in 1..len {
        // This only shuffles the first u32::MAX_VALUE - 1 elements.
        let r = range(xs, 0..i + 1);
        let i = i;
        swap(i, r);
    }
}
